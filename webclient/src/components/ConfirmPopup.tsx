...
export default function ConfirmPopup({ kind, onCancel, onConfirm }: {
  ...
  const title = kind === "game" ? "Forfeit this game?" : "Leave this match?";
  return (
        ...
        <div className="popup-content">
          <div className="action-popup-header action-popup-header-start">
            ...
            <p className="action-popup-subtitle">
              {kind === "game" ? "Your opponents will win this game. You can continue with the next game if the match is not over." : "Your opponents will win the match. This cannot be undone."}
            </p>
          </div>
        </div>
        <div className="popup-footer action-controls">
          <button type="button" className="action-button action-button-primary action-button-safe" autoFocus onClick={onCancel}>Keep playing</button>
          <button type="button" className="action-button action-button-danger" onClick={onConfirm}>{kind === "game" ? "Forfeit game" : "Quit match"}</button>
        </div>
      </div>
    </dialog>
  );
